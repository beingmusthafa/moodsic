...
export function MusicCard({ music, isPlaying, isCurrentMusic, onPlay }: MusicCardProps) {
  return (
      ...
      <CardContent className="p-0">
        <div className="relative aspect-square overflow-hidden">
          ...
          <Button
            ...
            onClick={onPlay}
          >
            {isCurrentMusic && isPlaying ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6" />}
          </Button>
        </div>
        <div className="p-4">
          <h3 className="font-semibold truncate">{music.title}</h3>
          <p className="text-sm text-muted-foreground truncate">{music.artists.join(", ")}</p>
        </div>
      </CardContent>
    </Card>
  )
